...
`site/` is a self-contained static website (HTML / CSS / vanilla JS — no build step).
...
```bash
node server.js        # serves ./site at http://localhost:5173
```
...
> Note: the fonts in `Type/` and `site/assets/fonts/` are **trial** versions — swap in licensed copies before any production use.
